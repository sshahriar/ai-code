...
- This is the beginning of our new project. Okay, so I'm going to bring up a terminal again with command and then the the backslash. And now, And now we're going to install anothe...
- And this time it's for GitHub. We want to connect to GitHub using GitHub's famous remote MCP server using the token that we just created and that you're taking great care to pre...
- And the reason is because first of all, these are two very famous MCP servers, Jura and GitHub that are very well known, very powerful and highly used in the community. And seco...
- Okay, so let's now connect to the github mcp server using our key. Okay, so I've pasted in here the command which is in the course resources and I close that so you can see it a...

## You will learn

- Core ideas from **Setting Up GitHub MCP Server & Featured Dev Plugin in Claude Code**

var e=`# Claude Code Hooks Auto-Trigger Reviews with Events & Commands
...
- So a hook is something which in a certain circumstance can kick off making Claude code do something. And those circumstances are called events and they're things like when Claud...
- It's moments like that, they're considered events and they can trigger a hook. They can say, "Okay, something's happened and I want to make sure that Claude Code always, when th...
- It always goes back to Python, which is the older pre UV way of doing it. You could add a hook so that before it calls a tool, any shell script, you always check if it's doing P...
- That would be an example of a time where you might want to use a hook. Another one from Claude's documentation, they have one where if it's ever stopping to ask for your permiss...
- You would get some kind of notification. So that's a cool one. So things like that are time to use hooks.

## You will learn

- Core ideas from **Claude Code Hooks Auto-Trigger Reviews with Events & Commands**
